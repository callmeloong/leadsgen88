...
export default async function ComparePage({
  ...
  if (!p1 || !p2)
    return <div className="p-8 text-center">Players not found</div>;
  ...
  const { data: matches } = await supabase
    ...
    .select("*")
    .or(
      `and(player1Id.eq.${p1Id},player2Id.eq.${p2Id}),and(player1Id.eq.${p2Id},player2Id.eq.${p1Id})`
    )
  ...
  matches?.forEach((m: any) => {
    const score1 = m.player1Id === p1Id ? m.player1Score : m.player2Score;
    const score2 = m.player1Id === p1Id ? m.player2Score : m.player1Score;
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div className="text-center space-y-4 flex-1">
              ...
              <div>
                ...
                <Badge
                  variant="outline"
                  className="text-lg mt-2 border-blue-500 text-blue-400"
                >
                  ELO: {p1.elo}
                </Badge>
              </div>
            ...
            <div className="text-center space-y-4 flex-1">
              ...
              <div>
                <h2 className="text-3xl font-black uppercase text-red-500">
                  {p2.name}
                </h2>
                <Badge
                  variant="outline"
                  className="text-lg mt-2 border-red-500 text-red-500"
                >
                  ELO: {p2.elo}
                </Badge>
              </div>
                ...
                return (
                  <TableRow key={match.id} className="hover:bg-muted/50">
                    ...
                    <TableCell className="text-center font-mono text-xl">
                      <span
                        className={isDraw ? "text-zinc-400" : "text-blue-400"}
                      >
                        {match.player1Id === p1Id
                          ? match.player1Score
                          : match.player2Score}
                      ...
                      <span
                        className={isDraw ? "text-zinc-400" : "text-red-500"}
                      >
                        {match.player1Id === p1Id
                          ? match.player2Score
                          : match.player1Score}
