...
export default async function Home() {
  ...
  const validLiveMatches =
    liveMatches?.filter((match: any) => {
  ...
  // Fetch Open Challenges (Kèo Thơm)
      ...
      <header className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-border pb-6">
        ...
        <div className="flex items-center gap-4">
          {user ? (
            ...
            <a
              href="/login"
              className="px-4 py-2 bg-secondary text-secondary-foreground rounded hover:bg-secondary/80 font-bold uppercase tracking-wider"
            >
              Login
            </a>
          )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="border-border bg-card/50">
            <CardHeader className="pb-2 flex flex-row items-center justify-between space-y-0">
              ...
              {currentPlayer && <OpenChallengeButton />}
            </CardHeader>
            <CardContent>
              <OpenChallengeList
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-primary/20">
                    <TableHead className="w-[80px] font-black text-primary">
                      RANK
                    </TableHead>
                ...
                <TableBody>
                  {players.map((player: any, index: number) => {
                    ...
                    return (
                      <TableRow
                        key={player.id}
                        className="text-2xl hover:bg-muted/50 transition-colors border-primary/10 group relative"
                      >
                          ...
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-4">
                              ...
                              <Link
                                ...
                                className="font-bold tracking-wide uppercase hover:underline hover:text-primary transition-colors"
                              >
                                <PlayerNameDisplay
                                  name={player.name}
                                  nickname={player.nickname}
                                  placement={player.nickname_placement}
                                />
                              </Link>
                            </div>
